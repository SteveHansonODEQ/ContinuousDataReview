...
for (m in 1:length(datafiles)) { 
  ...
  if (sum(tmp_data$r4calc, na.rm= TRUE) == 0) {
    validdatafiles <- validdatafiles[-m]
    print(paste0('No data: ', fname))
  }
...
for (i in 1:length(validdatafiles)) { 
  fname <- validdatafiles[i]#"XXXX_99990_10429625_DO_20150420_Salmon R Hatchery_1_.Rdata"
  load(fname)
